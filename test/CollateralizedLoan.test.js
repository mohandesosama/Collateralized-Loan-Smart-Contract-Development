...
describe("CollateralizedLoan", function () {
  ...
  it("Should deposit collateral", async function () {
    ...
    await collateralizedLoan.connect(addr1).depositCollateral(3600, { value: parseEther("1") });
    const loan = await collateralizedLoan.loans(addr1.address);
    expect(loan.borrower).to.equal(addr1.address);
    expect(loan.collateralAmount).to.equal(parseEther("1"));
  });
  ...
  it("Should fund loan", async function () {
    ...
    const parseEther = ethers.utils.parseEther;
    await collateralizedLoan.connect(addr1).depositCollateral(3600, { value: parseEther("1") });
    await collateralizedLoan.connect(owner).fundLoan(addr1.address, { value: parseEther("2") });
    const loan = await collateralizedLoan.loans(addr1.address);
  ...
  it("Should repay loan", async function () {
    ...
    await collateralizedLoan.connect(addr1).depositCollateral(3600, { value: parseEther("1") });
    await collateralizedLoan.connect(owner).fundLoan(addr1.address, { value: parseEther("2") });
    await collateralizedLoan.connect(addr1).repayLoan({ value: parseEther("2.2") });
    const loan = await collateralizedLoan.loans(addr1.address);
